...
- Prefer `subprocess` for shell commands; use Python for control flow. This makes commands easy to read and copy-paste elsewhere.
    ...
    - Where it would benefit the user to see what is being executed (e.g. long-running or destructive commands), use `_run()` instead of `subprocess.run()` directly — it prints the command in `bold cyan` before executing:
        ```python
